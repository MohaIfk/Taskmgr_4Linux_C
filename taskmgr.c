...
static const char *get_state_name(char state) {
    switch (state) { case 'R': return "Running"; case 'S': return "Sleeping"; case 'D': return "Uninterruptible Sleep"; case 'T': return "Stopped"; case 't': return "Tracing Stop"; case 'Z': return "Zombie"; case 'X': return "Dead"; case 'I': return "Idle"; default:  return "Unknown"; }
}

/* fast check whether a string is a positive integer (PID) */
static int is_numeric_str(const char *s) {
    if (!s || !*s) return 0;
    for (const char *p = s; *p; ++p) if (!isdigit((unsigned char)*p))
        return 0;
    return 1;
}

/* Print PID info (PID, comm, state) reading /proc/<pid>/stat safely using getline */
static void print_pid_info(const char *pid_str) {
    char path[64]; FILE *fp = NULL; char *line = NULL; size_t linecap = 0; ssize_t linelen;
...
int main(void) {
    struct dirent **namelist = NULL; int n = 0;
